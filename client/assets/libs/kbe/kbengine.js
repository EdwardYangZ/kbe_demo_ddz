...
KBEngine.Class.extend = cc.Class
// export
// global.Class = KBEngine.Class;

let parse = function (v) {
	// cc.log('parse', typeof(v))
	return null
}
...
KBEngine.INT64 = function(lo, hi)
{
	...
	// cc.log('int64 lo', this.lo)
	// cc.log('int64 hi', this.hi)
...
{
	app.kbeM.log(s);
	// app.kbeM.emit('kbengine|info', s)
}

KBEngine.DEBUG_MSG = function(s)
{
	// app.kbeM.log(s);
	// app.kbeM.emit('kbengine|log', s)
}
...
KBEngine.ERROR_MSG = function(s)
{
	/* CHANGE */
	app.kbeM.error(s);
	// app.kbeM.emit('kbengine|error', s)
}

KBEngine.WARNING_MSG = function(s)
{
	app.kbeM.warn(s);
	// app.kbeM.emit('kbengine|warn', s)
}
...
KBEngine.Event = function()
{
	...
	{
		// var callbackfn = parse("classinst." + strCallback);
	...
	this.fire = function()
	{
		...
		/* CHANGE */
		require('kbeM').kbeM.handleEvent( evtName, ars)
	}
}
...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.reset = function()
	{
		...
		KBEngine.app.clientScriptVersion = "1.0.0";
		
		cc.log('reset kbengine')
		// player的相关信息
	...
	this.connect = function(addr)
	{
		...
		try
		{
			cc.log(addr.indexOf('ws://1'))
			if (addr.indexOf('ws://1') < 0) {
				addr = addr.replace('ws:', 'wss:')
				addr = addr.replace(':'+KBEngine.app.port, '')
			}
			if (KBEngine.app.sslPem) {
				KBEngine.app.socket = new WebSocket(addr, '', KBEngine.app.sslPem);
			}else{
				KBEngine.app.socket = new WebSocket(addr);
			}
			KBEngine.INFO_MSG('connecting!'+addr);
		}
		catch(e)
		{  
	...
	{  
		KBEngine.ERROR_MSG('connect error:' + JSON.stringify(evt));
		KBEngine.app.resetSocket();
		KBEngine.Event.fire("onConnectionState", false);
	}
	...
	{
		KBEngine.ERROR_MSG('connect error:' + evt);
		let currserver = KBEngine.app.currserver
		KBEngine.app.resetSocket();
		KBEngine.Event.fire("onDisconnected", currserver);
	}
	...
	{  
		...
		// if(KBEngine.app.currserver != "loginapp")
		// 	KBEngine.app.reset();
		KBEngine.Event.fire("onDisconnected", KBEngine.app.currserver);
	}
	...
	*/
	...
	this.onImportClientMessagesCompleted = function()
	{
		...
		if(KBEngine.app.currserver == "loginapp")
		{
			...
			{
				KBEngine.DEBUG_MSG("KBEngine::onImportClientMessagesCompleted(): send importServerErrorsDescr!");
		...
		else
		{
			...
			{
				KBEngine.DEBUG_MSG("KBEngineApp::onImportClientMessagesCompleted: start importEntityDef ...");
				var bundle = new KBEngine.Bundle();
	...
	this.createDataTypeFromStreams = function(stream, canprint)
	{
		...
		KBEngine.DEBUG_MSG("KBEngineApp::createDataTypeFromStreams: importAlias(size=" + aliassize + ")!");
	...
	this.createDataTypeFromStream = function(stream, canprint)
	{
		...
		*/
		...
		if(canprint)
			KBEngine.DEBUG_MSG("KBEngineApp::Client_onImportClientEntityDef: importAlias(" + name + ":" + valname + ")!");
	...
	this.Client_onImportClientEntityDef = function(stream)
	{
		...
		while(!stream.readEOF())
		{
			...
			KBEngine.DEBUG_MSG("KBEngineApp::Client_onImportClientEntityDef: import(" + scriptmodule_name + "), propertys(" + propertysize + "), " +
			...
			try
			{
				...
				// var Class = parse("KBEngine." + scriptmodule_name);
			}
			catch(e)
			{
				var Class = undefined;
			...
			while(propertysize > 0)
			{
				...
				KBEngine.DEBUG_MSG("KBEngineApp::Client_onImportClientEntityDef: add(" + scriptmodule_name + "), property(" + name + "/" + properUtype + ").");
			};
			...
			while(methodsize > 0)
			{
				...
				KBEngine.DEBUG_MSG("KBEngineApp::Client_onImportClientEntityDef: add(" + scriptmodule_name + "), method(" + name + ").");
			};
			...
			while(base_methodsize > 0)
			{
				...
				KBEngine.DEBUG_MSG("KBEngineApp::Client_onImportClientEntityDef: add(" + scriptmodule_name + "), base_method(" + name + ").");
			};
			...
			while(cell_methodsize > 0)
			{
				...
				KBEngine.DEBUG_MSG("KBEngineApp::Client_onImportClientEntityDef: add(" + scriptmodule_name + "), cell_method(" + name + ").");
			};
			var defmethod;
			try
			{
				// defmethod = parse("KBEngine." + scriptmodule_name);
	...
	{
		KBEngine.DEBUG_MSG("KBEngineApp::onImportEntityDefCompleted: successfully!");
	...
	this.Client_onImportClientMessages = function(msg)
	{
		...
		if(msgid == KBEngine.messages.onImportClientMessages.id)
		{
			...
			KBEngine.DEBUG_MSG("KBEngineApp::onImportClientMessages: start(" + msgcount + ") ...!");
			...
			while(msgcount > 0)
			{
				...
				if(isClientMethod)
				{
					...
					{
						KBEngine.DEBUG_MSG("KBEngineApp::onImportClientMessages: import(" + msgname + ") successfully!");
					}
				}
	...
	{  
		if(noconnect)
		{
			...
			let adress = KBEngine.app.baseappIp
			if (adress.indexOf('1') != 0) {
				adress = adress + '/base' + KBEngine.app.baseappPort
			}else{
				adress = adress + ':' + KBEngine.app.baseappPort
			}
			KBEngine.app.connect("ws://" + adress);
	...
	this.onReOpenBaseapp = function()
	{
		...
		KBEngine.app.currserver = "baseapp";
		KBEngine.INFO_MSG(' onReOpenBaseapp entity_uuid '+ KBEngine.app.entity_uuid)
	...
	this.getentityclass = function(entityType)
	{
		...
		{
			// runclass = parse("KBEngine." + entityType);
	...
	{
		KBEngine.INFO_MSG("Client_onCreatedProxies: eid(" + eid + ")rndUUID("+ rndUUID +"), entityType(" + entityType + ")!");
	...
	this.onUpdatePropertys_ = function(eid, stream)
	{
		...
		if(entity == undefined)
		{
			...
			{
				/* CHANGE */
				KBEngine.INFO_MSG("KBEngineApp::Client_onUpdatePropertys: entity(" + eid + ") not found!");
				return;
			}
		...
		while(stream.length() > 0)
		{
			...
			// KBEngine.INFO_MSG("KBEngineApp::Client_onUpdatePropertys: " + entity.className + "(id=" + eid  + " " + propertydata[2] + ", val=" + val.toString() + ")!");
			KBEngine.INFO_MSG("onUpdatePropertys: " + entity.className + "(id=" + eid  + " " + propertydata[2] + ", val=" + val.toString() + ")!");
			...
			// app.kbeM.handleUpdProperty( propertydata[2], entity)
			if (entity.onUpdateProp) {
				entity.onUpdateProp(propertydata[2], oldval)
			}
		}
		
		/* 属性变化通知 */
		// cc.log('onUpdatePropertys', entity.className)
		// xx.event.emit('onUpdatePropertys', entity)
		/* CHANGE */
		// var kbeM = require('kbeM')
		// if (kbeM) {
		// 	kbeM.emit('onUpdatePropertys', entity)
		// }
	}
	...
	this.onRemoteMethodCall_ = function(eid, stream)
	{
		...
		if(entity == undefined)
		{
			// KBEngine.WARNING_MSG("KBEngineApp::Client_onRemoteMethodCall: entity(" + eid + ") not found!");
			return;
		}
		...
		{
			// KBEngine.WARNING_MSG("KBEngineApp::Client_onRemoteMethodCall: entity(" + eid + ") not found method(" + methoddata[2] + ")!");
		}

		/* 添加自定义事件触发 */
		// xx.event.emit(methoddata[2], args);
		require('kbeM').kbeM.handleEvent( methoddata[2], args)
	}
